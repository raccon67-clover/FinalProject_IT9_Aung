...
<head>
    ...
    <style>
        *{margin:0;padding:0;box-sizing:border-box;}
        body{font-family:Arial, Helvetica, sans-serif;background:#f4f6f9;display:flex;color:#222;}
        .sidebar{width:240px;height:100vh;background:#1f73e8;position:fixed;top:0;left:0;padding:30px 20px;color:white;display:flex;flex-direction:column;}
        .sidebar h2{font-size:24px;margin-bottom:6px;}
        .sidebar .staff-name{font-size:1rem;font-weight:bold;margin-bottom:20px;color:rgba(255,255,255,0.92);letter-spacing:0.2px;}
        .sidebar hr{border:none;height:1px;background:rgba(255,255,255,.4);margin-bottom:20px;}
        .side-nav{display:flex;flex-direction:column;gap:6px;margin-bottom:20px;}
        .nav-link{display:block;padding:10px 14px;border-radius:8px;color:white;background:transparent;font-size:0.95rem;cursor:pointer;text-decoration:none;transition:background 0.2s;border:none;text-align:left;width:100%;}
        .nav-link:hover{background:rgba(255,255,255,0.15);}
        .nav-link.active{background:rgba(255,255,255,0.25);font-weight:bold;}
        .logout{margin-top:auto;}
        .logout button{width:100%;padding:12px;border:none;border-radius:8px;background:white;color:#1f73e8;font-weight:bold;cursor:pointer;}
        .logout button:hover{background:#e8eefc;}
        .main-content{margin-left:240px;width:100%;padding:40px;}
        .alert-success{background:#e9f9ee;color:#1f8b43;padding:12px 16px;border-radius:8px;margin-bottom:24px;font-weight:bold;}
        .stats{display:flex;gap:20px;margin-bottom:30px;}
        .stat-card{flex:1;background:white;border-radius:12px;padding:24px;text-align:center;box-shadow:0 4px 12px rgba(0,0,0,.06);border-top:4px solid #ddd;}
        ...
        .section-box{background:white;border-radius:14px;padding:28px;box-shadow:0 4px 12px rgba(0,0,0,.06);margin-bottom:28px;}
        .section-box h2{font-size:1.4rem;font-weight:bold;margin-bottom:20px;color:#1a1a1a;}
        .course-card{background:white;border-radius:14px;padding:24px;box-shadow:0 4px 12px rgba(0,0,0,.06);margin-bottom:20px;}
        .course-header{display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:20px;flex-wrap:wrap;gap:1rem;}
        ...
        .meta-tag{background:#f8f9fc;border-radius:8px;padding:10px 16px;text-align:center;border:1px solid #eee;min-width:80px;}
        ...
        .btn-remove{background:#fee2e2;color:#dc3545;border:none;padding:6px 12px;border-radius:6px;cursor:pointer;font-size:0.82rem;font-weight:bold;}
        .btn-remove:hover{background:#dc3545;color:white;}
        .btn-approve{background:#28a745;color:white;border:none;padding:6px 12px;border-radius:6px;cursor:pointer;font-size:0.82rem;}
        .btn-approve:hover{background:#218838;}
        .btn-reject{background:#fee2e2;color:#dc3545;border:none;padding:6px 12px;border-radius:6px;cursor:pointer;font-size:0.82rem;font-weight:bold;}
        .btn-reject:hover{background:#dc3545;color:white;}
        .empty-state{text-align:center;padding:2rem;color:#aaa;font-size:0.9rem;}
        .report-grid{display:grid;grid-template-columns:repeat(auto-fit, minmax(200px,1fr));gap:16px;margin-bottom:28px;}
        .report-card{background:#f8f9fc;border-radius:10px;padding:20px;border:1px solid #eee;text-align:center;}
        ...
        .btn-print{background:#dc3545;color:white;border:none;padding:10px 20px;border-radius:8px;font-size:14px;font-weight:bold;cursor:pointer;}
        .btn-print:hover{background:#b02a37;}
        .print-only{display:none;}
        .content-box{background:#f8f9fc;border:1px solid #eee;border-radius:10px;padding:16px;margin-bottom:20px;}
        .content-form{display:grid;grid-template-columns:1fr 1fr 110px auto;gap:10px;align-items:start;margin-bottom:16px;}
        .content-form input,.content-form textarea,.edit-content-form input,.edit-content-form textarea{padding:10px;border:1px solid #ddd;border-radius:8px;font-family:Arial, Helvetica, sans-serif;}
        .content-form textarea{grid-column:1 / -1;min-height:80px;resize:vertical;}
        .edit-content-form{display:grid;grid-template-columns:1fr 1fr 110px auto auto;gap:10px;align-items:start;}
        .edit-content-form textarea{grid-column:1 / -1;min-height:80px;resize:vertical;}
        .content-actions{display:flex;gap:8px;align-items:center;flex-wrap:wrap;}
        .url-cell{max-width:260px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
        @media print{
            .sidebar,#section-pending,#section-courses,.stats,.alert-success,.btn-print{display:none !important;}
            body{background:white;display:block;}
            .main-content{margin-left:0;padding:0;}
            .section-box{box-shadow:none !important;border:none !important;border-radius:0 !important;padding:0 !important;margin:0 !important;}
            .print-only{display:block !important;}
            table{border-collapse:collapse;width:100%;}
            th,td{border:1px solid #bbb !important;padding:9px 12px !important;}
            thead th{background:#1f73e8 !important;color:white !important;-webkit-print-color-adjust:exact;print-color-adjust:exact;}
            .report-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:20px;}
            .report-card{border:1px solid #ccc !important;-webkit-print-color-adjust:exact;print-color-adjust:exact;}
            .progress-bar-fill,.badge{-webkit-print-color-adjust:exact;print-color-adjust:exact;}
        ...
        @media(max-width:900px){
            ...
            .content-form,.edit-content-form{grid-template-columns:1fr;}
            .content-form textarea,.edit-content-form textarea{grid-column:auto;}
        }
    </style>
</head>
<body>
...
<div class="main-content">
    ...
    @php
        ...
        $unenrollRequests = $courses->flatMap(fn($c) => $c->enrollments->where('status', 'unenroll_pending'));
    ...
    <div class="stats">
        ...
        <div class="stat-card red">
            <h3>Pending</h3>
            <p>{{ $pendingAll->count() + $unenrollRequests->count() }}</p>
        </div>
    </div>
    ...
    <div class="section-wrapper" id="section-pending">
        ...
        <div class="section-box">
            <h2>Unenroll Requests</h2>

            @if($unenrollRequests->count() > 0)
                <table>
                    <thead>
                        <tr>
                            <th>Student</th>
                    ...
                    <tbody>
                        @foreach($unenrollRequests as $enrollment)
                            <tr>
                                <td>
                                    <strong>{{ $enrollment->user->name }}</strong><br>
                                    <span style="font-size:0.78rem;color:#888;">{{ $enrollment->user->email }}</span>
                                ...
                                <td>
                                    <form action="{{ route('staff.enrollments.destroy', $enrollment->id) }}" method="POST"
                                        onsubmit="return confirm('Approve unenroll request and remove this enrollment?')">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="btn-remove">Approve Unenroll</button>
                                    </form>
                                </td>
                            </tr>
                        @endforeach
            ...
            @else
                <div class="empty-state">No unenroll requests.</div>
            @endif
        </div>
    </div>
        ...
        @forelse($courses as $course)
            <div class="course-card">
                ...
                <div class="content-box">
                    ...
                    <form method="POST" action="{{ route('staff.course-contents.store', $course->id) }}" class="content-form">
                        @csrf
                        ...
                        <button type="submit" class="btn-approve" style="height:40px;">Add Content</button>

                        <textarea name="body" placeholder="Lesson content"></textarea>
                    </form>
                        ...
                        <table>
                            ...
                            <tbody>
                                @foreach($course->contents as $content)
                                    <tr id="content-view-{{ $content->id }}">
                                        <td>{{ $content->sort_order }}</td>
                                        <td><strong>{{ $content->title }}</strong></td>
                                        <td style="color:#666;">{{ $content->body ?: 'No lesson text.' }}</td>
                                        <td class="url-cell">
                                            @if($content->video_url)
                                                <a href="{{ $content->video_url }}" target="_blank" style="color:#1f73e8;font-weight:bold;text-decoration:none;">
                                                    {{ $content->video_url }}
                                                </a>
                                            @else
                                        ...
                                        <td>
                                            <div class="content-actions">
                                                <button type="button" class="btn-approve" onclick="toggleContentEdit({{ $content->id }})">Edit</button>

                                                <form method="POST" action="{{ route('staff.course-contents.destroy', $content->id) }}">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" class="btn-remove" onclick="return confirm('Delete this content?')">Delete</button>
                                                </form>
                                            </div>
                                        </td>
                                    </tr>

                                    <tr id="content-edit-{{ $content->id }}" style="display:none;">
                                        <td colspan="5">
                                            <form method="POST" action="{{ route('staff.course-contents.update', $content->id) }}" class="edit-content-form">
                                                @csrf
                                                @method('PUT')

                                                <input type="text" name="title" value="{{ $content->title }}" required>
                                                <input type="url" name="video_url" value="{{ $content->video_url }}" placeholder="YouTube URL">
                                                <input type="number" name="sort_order" value="{{ $content->sort_order }}" min="0">
                                                <button type="submit" class="btn-approve" style="height:40px;">Save</button>
                                                <button type="button" class="btn-reject" style="height:40px;" onclick="toggleContentEdit({{ $content->id }})">Cancel</button>

                                                <textarea name="body" placeholder="Lesson content">{{ $content->body }}</textarea>
                                            </form>
                                        </td>
                                    </tr>
                                @endforeach
...
<script>
    ...
    function toggleContentEdit(id) {
        const viewRow = document.getElementById('content-view-' + id);
        const editRow = document.getElementById('content-edit-' + id);
